...
describe('IncidentsRequestService', () => {
	...
	describe('getIncidentForStreet', () => {
		it('should call correct api endpoint', () => {
			service.getIncidentForStreet(1);
			expect(httpClientSpy.get).toHaveBeenCalledTimes(1);
			expect(httpClientSpy.get).toHaveBeenCalledWith('/api/incidents/street/1');
		});
		...
		it('should return the correct incident', async () => {
			...
			const result = await firstValueFrom(service.getIncidentForStreet(1));
